@extends('layouts.app')
@section('title', 'Edit BRD Document')
@section('page_title', 'Edit BRD Document')
...
@php
    $record = $task ?? $epic ?? $sprint ?? $feature ?? $blueprint ?? $brd ?? $erd ?? $fsd ?? ${str_replace('-','_','brd-documents')} ?? null;
    if(!$record) {
        // fallback heuristic for controller vars
        $varname = \Illuminate\Support\Str::camel(\Illuminate\Support\Str::singular('brd-documents'));
        $record = $$varname ?? null;
    }
@endphp
...
<div class="breadcrumb" style="justify-content: space-between;">
    <div>
        <a href="{{ route('brd-documents.index') }}">BRD Document List</a> <span>/</span> 
        @if($record)
        <a href="{{ route('brd-documents.show', $record) }}">{{ $record->name ?? $record->title ?? 'Details' }}</a> <span>/</span> 
        @endif
        <span>Edit</span>
    </div>
    @if($record)
    <a href="{{ route('brd-documents.show', $record) }}" class="btn-submit" style="background: #fff; color: #475569; border: 1px solid #e2e8f0; font-size: 0.85rem; padding: 6px 12px; text-decoration: none;">⬅️ Back</a>
    @else
...
<div class="card">
    @if($record)
    <form action="{{ route('brd-documents.update', $record) }}" method="POST">
        @csrf @method('PUT')
        <div class="form-group">
            <label class="form-label">BRD Code</label>
            <input type="text" name="brd_code" class="form-control" value="{{ $record->brd_code ?? '' }}" required>
        </div>
        <div class="form-group">
        ...
        <button type="submit" class="btn-submit">Update BRD Document</button>
    </form>
    @else
