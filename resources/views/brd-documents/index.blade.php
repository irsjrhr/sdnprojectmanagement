@extends('layouts.app')
@section('title','BRD Documents')
@section('page_title','BRD Documents')
...
<div class="toolbar">
    <div style="display:flex; gap: 10px; flex-wrap: wrap; align-items: center;">
        <input type="text" id="filterSearch" placeholder="Search BRD..." style="padding: 8px 12px; border-radius: 6px; border: 1px solid #cbd5e1; font-size: 0.85rem; outline: none; min-width: 200px;">
    ...
    @can('create brd')
    <a href="{{ route('brd-documents.create') }}" class="btn-primary">＋ New BRD</a>
    @endcan
</div>
...
$(document).ready(function() {
    ...
    function getFilterUrl(baseUrl = `{{ route('brd-documents.index_async') }}`) {
    ...
    function loadBrds(url) {
        $('#brd-table-body').html('<tr><td colspan="5"><div style="text-align:center; padding: 40px; color: #64748b;">Loading BRD documents...</div></td></tr>');
        
        $.ajax({
            url: url,
            type: 'GET',
            success: function(response) {
                $('#brd-table-body').html(response.html);
                $('#pagination-container').html(response.pagination);
            },
            error: function() {
                $('#brd-table-body').html('<tr><td colspan="5"><div style="text-align:center; padding: 40px; color: #ef4444;">Failed to load BRD documents.</div></td></tr>');
            }
        });
    }

    // Initial Load
    loadBrds(getFilterUrl());

    // Filter Events
    $('#btnSubmitFilter').on('click', function() { loadBrds(getFilterUrl()); });
    ...
    $('#filterSearch').on('keypress', function(e) {
        if(e.which === 13) loadBrds(getFilterUrl());
    });

    $('#perPageSelect').on('change', function() { loadBrds(getFilterUrl()); });

    $(document).on('click', '#pagination-container a', function(e) {
        e.preventDefault();
        loadBrds($(this).attr('href'));
    });
