@extends('layouts.app')
@section('title', 'BRD Document Details')
@section('page_title', 'BRD Document Details')
...
@php
    $record = $brdDocument ?? null;
@endphp

<div class="breadcrumb">
    <a href="{{ route('brd-documents.index') }}">BRD Document List</a> <span>/</span> <span>{{ $record->brd_code ?? '' }} - {{ $record->title ?? 'Details' }}</span>
</div>

@if($record)
<div class="toolbar" style="display: flex; justify-content: space-between;">
    <a href="{{ route('brd-documents.index') }}" class="btn-edit" style="background: #fff; color: #475569;">⬅️ Back to List</a>
    <a href="{{ route('brd-documents.edit', $record) }}" class="btn-edit">✏️ Edit BRD Document</a>
</div>

<div class="brd-page cover-page">
    <div class="doc-cover" style="display: flex; flex-direction: column; justify-content: center; align-items: center; background: linear-gradient(135deg, #1e3a8a 0%, #1e40af 100%); color: white; text-align: center; position: relative; overflow: hidden; box-shadow: inset 0 0 100px rgba(0,0,0,0.5); min-height: 850px;">
        ...
        <div style="z-index: 10; width: 100%; max-width: 800px; margin: 0 auto; display: flex; flex-direction: column; align-items: center;">
            ...
            <div style="margin-top: 3rem; padding-top: 2rem; border-top: 2px solid rgba(255,255,255,0.3); width: 70%;">
                ...
                <p style="font-size: 1.2rem; font-weight: 600; opacity: 0.9; margin-top: 1rem;">BRD Code: {{ $record->brd_code ?? '' }}</p>
            </div>
        </div>
...
<div class="brd-page">
    <div class="doc-body">
        <div class="doc-header">
            <div class="doc-title">{{ $record->brd_code ?? '' }} - {{ strtoupper($record->title ?? 'UNTITLED') }}</div>
            <div class="doc-meta">
