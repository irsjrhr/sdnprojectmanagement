...
@php
    $record = $task ?? $epic ?? $sprint ?? $feature ?? $blueprint ?? $brd ?? $erd ?? $fsd ?? ${str_replace('-','_','epics')} ?? null;
    if(!$record) {
        // fallback heuristic for controller vars
        $varname = \Illuminate\Support\Str::camel(\Illuminate\Support\Str::singular('epics'));
        $record = $$varname ?? null;
    }
@endphp

<div class="breadcrumb">
    <a href="{{ route('epics.index') }}">Epic List</a> <span>/</span> 
    @if($record)
    <a href="{{ route('epics.show', $record) }}">{{ $record->name ?? $record->title ?? 'Details' }}</a> <span>/</span> 
    @endif
    <span>Edit</span>
</div>
...
<div class="card">
    @if($record)
    <form action="{{ route('epics.update', $record) }}" method="POST">
        @csrf @method('PUT')
        ...
        <div style="display:flex; gap:20px; margin-bottom:20px;">
            <div style="flex:1;">
                ...
                <input type="date" name="start_date" class="form-control" value="{{ old('start_date', optional($record->start_date)->format('Y-m-d')) }}">
            </div>
            <div style="flex:1;">
                <label class="form-label">End Date</label>
                <input type="date" name="end_date" class="form-control" value="{{ old('end_date', optional($record->end_date)->format('Y-m-d')) }}">
            </div>
        </div>
        ...
        <button type="submit" class="btn-submit">Update Epic</button>
    </form>
    @else
