@extends('layouts.app')
@section('title', 'Edit ERD')
@section('page_title', 'Edit ERD')
...
<div class="breadcrumb">
    <a href="{{ route('erds.index') }}">ERD List</a> <span>/</span> <a href="{{ route('erds.show', $erd) }}">{{ $erd->code }}</a> <span>/</span> <span>Edit</span>
</div>

<div style="margin-bottom: 24px; max-width: 900px; margin-left: auto; margin-right: auto; display: flex; justify-content: space-between;">
    ...
    <a href="{{ route('erds.show', $erd) }}" style="display:inline-flex;align-items:center;gap:8px;padding:9px 18px;background:#f8fafc;color:#64748b;border:1px solid #cbd5e1;border-radius:10px;font-size:.9rem;font-weight:600;text-decoration:none;transition:all .2s" onmouseover="this.style.background='#f1f5f9'; this.style.color='#334155'" onmouseout="this.style.background='#f8fafc'; this.style.color='#64748b'">👁️ View Details</a>
</div>

<div class="form-card">
    <form action="{{ route('erds.update', $erd) }}" method="POST">
        @csrf
        @method('PUT')
        
        <div class="form-group">
            <label class="form-label">ERD Code</label>
            <input type="text" name="code" class="form-control" placeholder="e.g. ERD-001" required value="{{ old('code', $erd->code) }}">
        </div>
        
        <div class="form-group">
            <label class="form-label">Title</label>
            <input type="text" name="title" class="form-control" placeholder="e.g. Master Data Schema" required value="{{ old('title', $erd->title) }}">
        </div>

        <div class="form-group">
            <label class="form-label">Status</label>
            <select name="status" class="form-control" required>
                <option value="Draft" {{ old('status', $erd->status) == 'Draft' ? 'selected' : '' }}>Draft</option>
                <option value="Review" {{ old('status', $erd->status) == 'Review' ? 'selected' : '' }}>Review</option>
                <option value="Approved" {{ old('status', $erd->status) == 'Approved' ? 'selected' : '' }}>Approved</option>
            </select>
        </div>

        <div class="form-group">
            <label class="form-label">Description (Markdown Supported)</label>
            <textarea name="description" class="form-control" rows="4" placeholder="Brief explanation of this diagram...">{{ old('description', $erd->description) }}</textarea>
        </div>

        <div class="form-group">
            <label class="form-label">Document Content (HTML / Markdown)</label>
            <textarea name="content" class="form-control" rows="10" placeholder="<h3>[NEW] Table: users</h3>...">{{ old('content', $erd->content) }}</textarea>
        ...
        <div class="form-group">
            ...
            <textarea name="dbml" class="form-control code-editor" rows="20" placeholder="Table users {&#10;  id integer [primary key]&#10;  username varchar&#10;}">{{ old('dbml', $erd->dbml) }}</textarea>
        ...
        <div style="text-align: right; margin-top: 30px; border-top: 1px solid #f1f5f9; padding-top: 20px;">
            <button type="submit" class="btn-submit">💾 Update ERD</button>
        </div>
    </form>
</div>
@endsection