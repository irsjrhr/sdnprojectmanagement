...
<div class="breadcrumb">
    <a href="{{ route('project-features.index') }}">Feature List</a> <span>/</span> 
    @if($record)
    <a href="{{ route('project-features.show', $record) }}">{{ $record->name ?? $record->title ?? 'Details' }}</a> <span>/</span> 
    @endif
    <span>Edit</span>
</div>
...
<div class="card">
    @if($record)
    <form action="{{ route('project-features.update', $record) }}" method="POST">
        @csrf @method('PUT')
        ...
        <button type="submit" class="btn-submit">Update Feature</button>
    </form>
    @else
