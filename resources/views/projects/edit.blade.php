...
<div class="breadcrumb">
    ...
    <a href="{{ route('projects.show',$project) }}">{{ $project->name }}</a> <span>/</span>
    <span>Edit</span>
</div>
    ...
    <form method="POST" action="{{ route('projects.update',$project) }}">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="text" name="key" class="form-control" value="{{ old('key',$project->key) }}" required style="text-transform:uppercase" oninput="this.value=this.value.toUpperCase()">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="owner_id" class="form-control" required>
                    @foreach($users as $u)
                    <option value="{{ $u->id }}" {{ old('owner_id',$project->owner_id)==$u->id?'selected':'' }}>{{ $u->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
        ...
        <div style="display:flex;gap:8px;margin-top:8px">
            <a href="{{ route('projects.show',$project) }}" class="btn-back">← Back</a>
