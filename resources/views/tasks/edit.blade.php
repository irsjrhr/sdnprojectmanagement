...
<div class="breadcrumb">
    ...
    <a href="{{ route('tasks.show', $task) }}">{{ $task->title }}</a> <span>/</span> 
    <span>Edit</span>
</div>
                    ...
                    <div class="col form-group">
                        ...
                        <select name="brd_document_id" class="form-control">
                            ...
                            @foreach($brdDocuments as $brd)
                                <option value="{{ $brd->id }}" {{ old('brd_document_id', $task->brd_document_id) == $brd->id ? 'selected' : '' }}>{{ $brd->brd_code }} - {{ $brd->title }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col form-group">
                        ...
                        <select name="epic_id" class="form-control">
                            ...
                            @foreach($epics as $epic)
                                <option value="{{ $epic->id }}" {{ old('epic_id', $task->epic_id) == $epic->id ? 'selected' : '' }}>{{ $epic->name }} ({{ $epic->project->name ?? '-' }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col form-group">
            ...
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <select name="priority" class="form-control" required>
                        @foreach(['Highest', 'High', 'Medium', 'Low', 'Lowest'] as $priority)
                ...
                <div class="form-group">
                    ...
                    <select name="type" class="form-control" required>
                        @foreach(['Bug', 'Story', 'Task', 'Subtask'] as $type)
                            <option value="{{ $type }}" {{ old('type', $task->type) == $type ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="row">
                    <div class="col form-group">
                        ...
                        <input type="number" name="story_points" class="form-control" value="{{ old('story_points', $task->story_points) }}">
                    </div>
                    <div class="col form-group">
                        <label class="form-label">Est. Hours</label>
                        <input type="number" step="0.1" name="estimated_hours" class="form-control" value="{{ old('estimated_hours', $task->estimated_hours) }}">
                    </div>
                </div>
